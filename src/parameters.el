					;-*-Lisp-*-
; This is the parameters file for the code.  Anything that needs changing can be done here. (Apart from the number of iterations).
...
(setq number-of-offspring-survivors 8)
; Percentage of top scoring files carried forward, the rest will be randomly selected from the other values.
; This is out of 100.
(setq percentage-of-top-scorers 60)
...
(setq starting-noise-files (+ number-of-offspring-survivors randomly-generated-files-per-generation))
...
(setq mutator-plus-minus-repeats 15) ; amount of times to re-run the splicer code over all files.
...
(setq mutator-swapper-repeats 1) ; amount of times to re-run the swapper code over all files.
