...
int main(int argc, char* argv[])
{
     ...
     short int bufferin1[FILE_SIZE],newbuffer[BUF_SIZE];
     int pos,randseed;
     ...
     // CAN'T REMEMBER HOW TO SPECIFY FLOATS!!
     ...
     else
     {
         ...
         chance_of_change = atoi(argv[4]);
          random_change_var = atoi(argv[5]);
         ...
         for(k=HEADER_SIZE;k<FILE_SIZE;k++)
         {
            ...
            change_chance = randshuffler(5,100);
            currentval = bufferin1[k];
             if (change_chance <= chance_of_change)
             {
                //The new value moved up or down a random amount:
                random_change = randshuffler(5,(2*random_change_var));
                top_value = 32767;
                bottom_value = -32767;
                top_distance = top_value-currentval;
                bottom_distance = currentval - bottom_value;
                if(top_distance < random_change_var)
                {
                    lowest_random = currentval - ((random_change_var*2)-top_distance);
                }
                else if (bottom_distance < random_change_var)
                {
                    lowest_random = bottom_value;
                }
                else
                {
                    lowest_random = currentval - random_change_var;
                }

                newval = lowest_random + random_change;

                if (newval>top_value)
                {
                    printf("ERROR: Too high value!");
                }
                else if (newval<bottom_value)
                {
